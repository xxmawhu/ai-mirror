...
size_t Merger::Merge(std::vector<std::string>* NewFiles)
{
    ...
    for (size_t i = NumFilesInFirstCorpus; i < Files.size(); i++)
    {
        ...
        std::set_difference(Cur.begin(), Cur.end(), AllFeatures.begin(), AllFeatures.end(), std::inserter(Tmp, Tmp.begin()));
        Cur.swap(Tmp);
    }
    ...
    std::sort(Files.begin() + NumFilesInFirstCorpus, Files.end(), [&](const MergeFileInfo& a, const MergeFileInfo& b) -> bool {
        if (a.Size != b.Size)
