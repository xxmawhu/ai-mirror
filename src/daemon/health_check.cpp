...
std::vector<HealthStatus> HealthCheck::check_all() {
  ...
  for (const auto &m : issues) {
    ...
    if (!m.active) {
      utils::get_logger()->warn("Unhealthy mount: {} -> {}", m.source.string(),
                                m.target.string());
    }
  }
...
int HealthCheck::run_periodic(int interval_seconds) {
  ...
  utils::get_logger()->info("Health check running every {} seconds",
                            interval_seconds);
  ...
  while (running_.load() && !signal_received_.load()) {
    ...
    if (unhealthy > 0) {
      utils::get_logger()->warn("Health check: {}/{} mounts unhealthy",
                                unhealthy, statuses.size());
    } else {
      utils::get_logger()->debug("Health check: all {} mounts healthy",
                                 statuses.size());
    }

    std::unique_lock<std::mutex> lock(mtx_);
    cv_.wait_for(lock, std::chrono::seconds(interval_seconds), [this] {
      return !running_.load() || signal_received_.load();
    });
  }
