...
template <typename Value> struct printer {
  ...
  std::string operator()(const string_type &x) const { return "\"" + x + "\""; }
...
struct type_config_ordered_map {
  ...
  template <typename K, typename T> using table_type = std::map<K, T>;
...
TEST_CASE("testing toml::get with toml types") {
  ...
  }
  {
    ...
    CHECK_EQ("[true, 42, " + std::to_string(3.14) + ", \"foo\", ]",
             toml::visit(p, v));
  }
  {
