...
TEST_CASE("testing toml::find with toml type") {
  ...
  }
  {
    ...
    offset_datetime_type x =
        toml::find<offset_datetime_type>(std::move(v), "a");
    CHECK_EQ(dt, x);
  }
  {
    array_type vec;
...
TEST_CASE("testing find throws") {
...

  {
    // value is not a table
    toml::value v(true);
    CHECK_THROWS_AS(toml::find<boolean_type>(std::move(v), "key"),
                    toml::type_error);
  }
  {
    // the value corresponding to the key is not the expected type
...
TEST_CASE("testing toml::find(v, idx) throws") {
  ...
  // move version
  {
    // value is not an array
    toml::value v(true);
    CHECK_THROWS_AS(toml::find<boolean_type>(std::move(v), 0),
                    toml::type_error);
  }
  {
    // the value corresponding to the key is not the expected type
  ...
  }
  {
    // the value corresponding to the key is not found
    toml::value v = toml::array{1, 2, 3, 4, 5};
    CHECK_THROWS_AS(toml::find<integer_type>(std::move(v), 6),
                    std::out_of_range);
  }
  {
    // the positive control.
...
TEST_CASE("testing toml::find with recursive table/array") {
  ...
  // recursively search tables
  {
    toml::value v = toml::table{
        {"a", toml::table{{"b", toml::table{{"c", toml::table{{"d", 42}}}}}}}};
  ...
  {
    toml::value v =
        toml::array{toml::array{"array", "of", "string"},
...
TEST_CASE("testing toml::find with toml types") {
  ...
  {
    offset_datetime_type dt(
        local_datetime_type(toml::local_date(2018, toml::month_t::Apr, 22),
                            local_time_type(12, 30, 45)),
        toml::time_offset(9, 0));
    value_type v = toml::table{{"key", dt}};
...
TEST_CASE("testing toml::find table conversion") {
  using value_type = toml::value;
  {
    value_type v1 = toml::table{
        {"key",
         toml::table{{"key1", 1}, {"key2", 2}, {"key3", 3}, {"key4", 4}}}};
...
TEST_CASE("testing toml::find local_date") {
  ...
  {
    value_type v1 =
        toml::table{{"key", toml::local_date{2018, toml::month_t::Apr, 1}}};
...
TEST_CASE("testing toml::find local_time") {
  using value_type = toml::value;
  {
    ...
    CHECK_EQ(time, std::chrono::hours(12) + std::chrono::minutes(30) +
