...
int main() {
  ...
  // using member functions
  {
    ...
    assert(root.at("database").at("temp_targets").at("cpu").as_floating() ==
           79.5);
    assert(root.at("database").at("temp_targets").at("case").as_floating() ==
           72.0);
  ...
  // using toml::find
  {
    ...
    assert(toml::find<std::string>(root, "owner", "name") ==
           "Tom Preston-Werner");
