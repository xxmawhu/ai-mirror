...
std::optional<fs::path> PathResolver::resolve(const fs::path &p) {
  ...
  if (p.string().substr(0, 2) == "~/") {
    expanded = fs::path(utils::get_home_dir(utils::get_effective_username())) /
               p.string().substr(2);
  } else if (p.string() == "~") {
...
fs::path PathResolver::to_ai_user_path(const fs::path &main_path,
                                       const std::string &ai_user,
                                       const std::string &main_user,
                                       const fs::path &ai_user_home) {
  std::string main_home = utils::get_effective_home();
  if (main_home.empty()) {
    main_home = utils::get_home_dir(main_user);
  }
  std::string ai_home = ai_user_home.empty() ? utils::get_home_dir(ai_user)
                                             : ai_user_home.string();
...
std::string PathResolver::detect_ai_user_from_path(const fs::path &p,
                                                   const std::string &main_user,
                                                   const std::string &prefix) {
  if (main_user.empty() || prefix.empty())
    return "";

  std::string expected_prefix = prefix + main_user + "_";
  std::error_code ec;

  // Check if a username matches AI user format and exists in system (via
  // getpwnam) This works for any home directory location (including BeeGFS,
  // NFS, etc.)
  auto check_component = [&](const fs::path &component) -> std::string {
    ...
    if (name.length() <= expected_prefix.length())
      return "";
    if (name.substr(0, expected_prefix.length()) != expected_prefix)
      return "";
    if (!utils::validate_username(name))
      return "";
...
std::optional<fs::path> PathResolver::find_project_root(const fs::path &p) {
  ...
  while (!current.empty() && current != "/") {
    if (fs::exists(current / ".git") ||
        fs::exists(current / ".ai-mirror.toml")) {
      return current;
    }
...
bool PathResolver::is_under_project(const fs::path &path,
                                    const fs::path &project_root) {
  auto resolved_path_opt = resolve(path);
  auto resolved_root_opt = resolve(project_root);
  if (!resolved_path_opt || !resolved_root_opt)
    return false;
