...
bool check_path_permissions(const fs::path &p,
...
AuditReport audit_user_permissions(const std::string &username) {
  ...
  report.add("user", "no_shell_login", check_no_shell_login(username),
             check_no_shell_login(username) ? "restricted shell"
                                            : "WARNING: login shell enabled");
  report.add("user", "home_owner", check_home_owner(username),
             username + " home directory ownership");
...
AuditReport full_audit() {
  ...
  report.add("system", "root_check", !ai_mirror::utils::is_root(),
             ai_mirror::utils::is_root() ? "WARNING: running as root"
                                         : "not running as root");
  ...
  for (auto &entry : config_report.entries) {
    report.add("config:" + entry.category, entry.item, entry.passed,
               entry.detail);
  }
  ...
  if (mounts_result.exit_code == 0) {
    ...
    int bind_count = 0;
    while (std::getline(stream, line)) {
      if (line.find("fuse.bind") != std::string::npos ||
...
bool write_audit_report(const AuditReport &report,
  ...
  for (const auto &entry : report.entries) {
    ofs << "[" << (entry.passed ? "PASS" : "FAIL") << "] " << entry.category
        << "/" << entry.item;
