...
void test_is_type(const toml::basic_value<TC> &v, const toml::value_t t) {
  ...
  if (t == toml::value_t::boolean) {
    CHECK_UNARY(v.is_boolean());
  } else {
    CHECK_UNARY_FALSE(v.is_boolean());
  }
  if (t == toml::value_t::integer) {
    CHECK_UNARY(v.is_integer());
  } else {
    CHECK_UNARY_FALSE(v.is_integer());
  }
  if (t == toml::value_t::floating) {
    CHECK_UNARY(v.is_floating());
  } else {
    CHECK_UNARY_FALSE(v.is_floating());
  }
  if (t == toml::value_t::string) {
    CHECK_UNARY(v.is_string());
  } else {
    CHECK_UNARY_FALSE(v.is_string());
  }
  if (t == toml::value_t::offset_datetime) {
    CHECK_UNARY(v.is_offset_datetime());
  } else {
    CHECK_UNARY_FALSE(v.is_offset_datetime());
  }
  if (t == toml::value_t::local_datetime) {
    CHECK_UNARY(v.is_local_datetime());
  } else {
    CHECK_UNARY_FALSE(v.is_local_datetime());
  }
  if (t == toml::value_t::local_date) {
    CHECK_UNARY(v.is_local_date());
  } else {
    CHECK_UNARY_FALSE(v.is_local_date());
  }
  if (t == toml::value_t::local_time) {
    CHECK_UNARY(v.is_local_time());
  } else {
    CHECK_UNARY_FALSE(v.is_local_time());
  }
  if (t == toml::value_t::array) {
    CHECK_UNARY(v.is_array());
  } else {
    CHECK_UNARY_FALSE(v.is_array());
  }
  if (t == toml::value_t::table) {
    CHECK_UNARY(v.is_table());
  } else {
    CHECK_UNARY_FALSE(v.is_table());
  }
  if (t == toml::value_t::empty) {
    CHECK_UNARY(v.is_empty());
  } else {
    CHECK_UNARY_FALSE(v.is_empty());
  }
  ...
  if (t == toml::value_t::boolean) {
    CHECK_UNARY(v.template is<boolean_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<boolean_type>());
  }
  if (t == toml::value_t::integer) {
    CHECK_UNARY(v.template is<integer_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<integer_type>());
  }
  if (t == toml::value_t::floating) {
    CHECK_UNARY(v.template is<floating_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<floating_type>());
  }
  if (t == toml::value_t::string) {
    CHECK_UNARY(v.template is<string_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<string_type>());
  }
  if (t == toml::value_t::offset_datetime) {
    CHECK_UNARY(v.template is<offset_datetime_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<offset_datetime_type>());
  }
  if (t == toml::value_t::local_datetime) {
    CHECK_UNARY(v.template is<local_datetime_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<local_datetime_type>());
  }
  if (t == toml::value_t::local_date) {
    CHECK_UNARY(v.template is<local_date_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<local_date_type>());
  }
  if (t == toml::value_t::local_time) {
    CHECK_UNARY(v.template is<local_time_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<local_time_type>());
  }
  if (t == toml::value_t::array) {
    CHECK_UNARY(v.template is<array_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<array_type>());
  }
  if (t == toml::value_t::table) {
    CHECK_UNARY(v.template is<table_type>());
  } else {
    CHECK_UNARY_FALSE(v.template is<table_type>());
  }
  //     if(t == toml::value_t::empty          ) {CHECK_UNARY(v.template is</*no
  //     such type!*/>());} else {CHECK_UNARY_FALSE(v.template is</*no such
  //     type!*/>()   );}
...
void test_as_type_throws(toml::basic_value<TC> &v, const toml::value_t t) {
  if (t != toml::value_t::boolean) {
    CHECK_THROWS_AS(v.as_boolean(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::boolean>(), toml::type_error);
  }
  if (t != toml::value_t::integer) {
    CHECK_THROWS_AS(v.as_integer(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::integer>(), toml::type_error);
  }
  if (t != toml::value_t::floating) {
    CHECK_THROWS_AS(v.as_floating(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::floating>(), toml::type_error);
  }
  if (t != toml::value_t::string) {
    CHECK_THROWS_AS(v.as_string(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::string>(), toml::type_error);
  }
  if (t != toml::value_t::offset_datetime) {
    CHECK_THROWS_AS(v.as_offset_datetime(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::offset_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_datetime) {
    CHECK_THROWS_AS(v.as_local_datetime(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::local_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_date) {
    CHECK_THROWS_AS(v.as_local_date(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::local_date>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_time) {
    CHECK_THROWS_AS(v.as_local_time(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::local_time>(),
                    toml::type_error);
  }
  if (t != toml::value_t::array) {
    CHECK_THROWS_AS(v.as_array(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::array>(), toml::type_error);
  }
  if (t != toml::value_t::table) {
    CHECK_THROWS_AS(v.as_table(), toml::type_error);
    CHECK_THROWS_AS(v.template as<toml::value_t::table>(), toml::type_error);
  }

  if (t != toml::value_t::boolean) {
    CHECK_THROWS_AS(as_const(v).as_boolean(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::boolean>(),
                    toml::type_error);
  }
  if (t != toml::value_t::integer) {
    CHECK_THROWS_AS(as_const(v).as_integer(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::integer>(),
                    toml::type_error);
  }
  if (t != toml::value_t::floating) {
    CHECK_THROWS_AS(as_const(v).as_floating(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::floating>(),
                    toml::type_error);
  }
  if (t != toml::value_t::string) {
    CHECK_THROWS_AS(as_const(v).as_string(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::string>(),
                    toml::type_error);
  }
  if (t != toml::value_t::offset_datetime) {
    CHECK_THROWS_AS(as_const(v).as_offset_datetime(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::offset_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_datetime) {
    CHECK_THROWS_AS(as_const(v).as_local_datetime(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::local_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_date) {
    CHECK_THROWS_AS(as_const(v).as_local_date(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::local_date>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_time) {
    CHECK_THROWS_AS(as_const(v).as_local_time(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::local_time>(),
                    toml::type_error);
  }
  if (t != toml::value_t::array) {
    CHECK_THROWS_AS(as_const(v).as_array(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::array>(),
                    toml::type_error);
  }
  if (t != toml::value_t::table) {
    CHECK_THROWS_AS(as_const(v).as_table(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as<toml::value_t::table>(),
                    toml::type_error);
  }
...
template <toml::value_t VT> struct as_type_tester;
#ifndef TOML11_TEST_DEFINE_AS_TYPE_TESTER
#define TOML11_TEST_DEFINE_AS_TYPE_TESTER(ty)                                  \
  template <> struct as_type_tester<toml::value_t::ty> {                       \
...
void test_as_type_fmt_throws(toml::basic_value<TC> &v, const toml::value_t t) {
  if (t != toml::value_t::boolean) {
    CHECK_THROWS_AS(v.as_boolean_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::boolean>(),
                    toml::type_error);
  }
  if (t != toml::value_t::integer) {
    CHECK_THROWS_AS(v.as_integer_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::integer>(),
                    toml::type_error);
  }
  if (t != toml::value_t::floating) {
    CHECK_THROWS_AS(v.as_floating_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::floating>(),
                    toml::type_error);
  }
  if (t != toml::value_t::string) {
    CHECK_THROWS_AS(v.as_string_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::string>(),
                    toml::type_error);
  }
  if (t != toml::value_t::offset_datetime) {
    CHECK_THROWS_AS(v.as_offset_datetime_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::offset_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_datetime) {
    CHECK_THROWS_AS(v.as_local_datetime_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::local_datetime>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_date) {
    CHECK_THROWS_AS(v.as_local_date_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::local_date>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_time) {
    CHECK_THROWS_AS(v.as_local_time_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::local_time>(),
                    toml::type_error);
  }
  if (t != toml::value_t::array) {
    CHECK_THROWS_AS(v.as_array_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::array>(),
                    toml::type_error);
  }
  if (t != toml::value_t::table) {
    CHECK_THROWS_AS(v.as_table_fmt(), toml::type_error);
    CHECK_THROWS_AS(v.template as_fmt<toml::value_t::table>(),
                    toml::type_error);
  }

  if (t != toml::value_t::boolean) {
    CHECK_THROWS_AS(as_const(v).as_boolean_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::boolean>(),
                    toml::type_error);
  }
  if (t != toml::value_t::integer) {
    CHECK_THROWS_AS(as_const(v).as_integer_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::integer>(),
                    toml::type_error);
  }
  if (t != toml::value_t::floating) {
    CHECK_THROWS_AS(as_const(v).as_floating_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::floating>(),
                    toml::type_error);
  }
  if (t != toml::value_t::string) {
    CHECK_THROWS_AS(as_const(v).as_string_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::string>(),
                    toml::type_error);
  }
  if (t != toml::value_t::offset_datetime) {
    CHECK_THROWS_AS(as_const(v).as_offset_datetime_fmt(), toml::type_error);
    CHECK_THROWS_AS(
        as_const(v).template as_fmt<toml::value_t::offset_datetime>(),
        toml::type_error);
  }
  if (t != toml::value_t::local_datetime) {
    CHECK_THROWS_AS(as_const(v).as_local_datetime_fmt(), toml::type_error);
    CHECK_THROWS_AS(
        as_const(v).template as_fmt<toml::value_t::local_datetime>(),
        toml::type_error);
  }
  if (t != toml::value_t::local_date) {
    CHECK_THROWS_AS(as_const(v).as_local_date_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::local_date>(),
                    toml::type_error);
  }
  if (t != toml::value_t::local_time) {
    CHECK_THROWS_AS(as_const(v).as_local_time_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::local_time>(),
                    toml::type_error);
  }
  if (t != toml::value_t::array) {
    CHECK_THROWS_AS(as_const(v).as_array_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::array>(),
                    toml::type_error);
  }
  if (t != toml::value_t::table) {
    CHECK_THROWS_AS(as_const(v).as_table_fmt(), toml::type_error);
    CHECK_THROWS_AS(as_const(v).template as_fmt<toml::value_t::table>(),
                    toml::type_error);
  }
...
template <toml::value_t VT> struct as_type_fmt_tester;
#ifndef TOML11_TEST_DEFINE_AS_TYPE_FMT_TESTER
#define TOML11_TEST_DEFINE_AS_TYPE_FMT_TESTER(ty)                              \
  template <> struct as_type_fmt_tester<toml::value_t::ty> {                   \
...
void test_ctors(T eq, U ne, FMT fmt) {
  ...
  // -----------------------------------------------------------------------
  {
    toml::value x_with_com_fmt(eq, fmt, std::vector<std::string>{"foo", "bar"});
...
TEST_CASE("testing constructor (system_clock::time_point)") {
  ...
  const std::chrono::system_clock::time_point eq = toml::offset_datetime(
      toml::local_date(2023, toml::month_t::Feb, 2),
      toml::local_time(12, 30, 45), toml::time_offset(0, 0));
  const std::chrono::system_clock::time_point ne = toml::offset_datetime(
      toml::local_date(2023, toml::month_t::Jan, 1),
      toml::local_time(2, 34, 56), toml::time_offset(-9, 0));
...
TEST_CASE("testing constructor (array-like)") {
  ...
  toml::value x_with_comments(std::deque<toml::value>{true, 42, "hoge"},
                              std::vector<std::string>{"foo", "bar"});
...
TEST_CASE("testing constructor (table-like)") {
  ...
  toml::value x(std::map<std::string, toml::value>{
      {"a", true}, {"b", 42}, {"c", "hoge"}});
  ...
  toml::value x_with_comments(
      std::map<std::string, toml::value>{{"a", true}, {"b", 42}, {"c", "hoge"}},
      std::vector<std::string>{"foo", "bar"});
  ...
  toml::value x_with_com_fmt(
      std::map<std::string, toml::value>{{"a", true}, {"b", 42}, {"c", "hoge"}},
      fmt, std::vector<std::string>{"foo", "bar"});
...
struct X {
  toml::value into_toml() const { return toml::value(this->value); }
  int value;
};
...
TEST_CASE("testing constructor (into_toml)") {
  ...
  toml::value x_with_comments(initialized_with,
                              std::vector<std::string>{"foo", "bar"});
