...
int main() {
  ...
  assert(root.comments().at(1) == " This contains most of the examples in the "
                                  "spec (from https://toml.io).");
  ...
  // using toml::find
  {
    ...
    assert(toml::find<std::string>(keys, "ʎǝʞ") ==
           "value"); // requires /utf-8 in MSVC
    ...
    assert(toml::find<std::string>(keys, "fruit", "orange", "color") ==
           "orange");
