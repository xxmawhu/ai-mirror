...
// - Fall under SYSTEM_DIRS blacklist (17 FHS directories; /mnt excluded for
// HPC/BeeGFS)
// - Are symlinks that resolve outside allowed boundaries
//
// safe_canonical() returns empty path on failure (non-existent, dangling
// symlink), callers must treat empty as rejection. No weakly_canonical fallback
// to prevent
// ".." bypass attacks.
//
...
// Returns empty path on failure (non-existent, symlink loop, permission
// denied). Callers must treat empty path as validation failure.
