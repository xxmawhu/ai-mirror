...
static std::unordered_map<uid_t, std::pair<int, unsigned long>>
scan_proc_by_uid() {
...
static PsAllData scan_ps_all() {
  ...
  auto comm_result =
      utils::exec_safe({"ps", "-eo", "comm,uid", "--no-headers"});
...
std::unordered_map<uid_t, UidStats> gather_all_uid_stats() {
  ...
  for (auto &[uid, stats] : result) {
    ...
    if (auto it = ps_data.cpu_by_uid.find(uid);
        it != ps_data.cpu_by_uid.end()) {
