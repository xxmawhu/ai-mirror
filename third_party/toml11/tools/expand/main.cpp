...
is_include(const std::string &line, const std::filesystem::path &filepath) {
  ...
  if (iter == line.end() || *iter != 'i') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'n') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'c') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'l') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'u') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'd') {
    return std::nullopt;
  } else {
    ++iter;
  }
  if (iter == line.end() || *iter != 'e') {
    return std::nullopt;
  } else {
    ++iter;
  }
  ...
  return std::filesystem::canonical(filepath.parent_path() /
...
struct File {
  ...
  explicit File(std::filesystem::path f) : filename(std::move(f)) {
...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    std::cerr << "Usage: ./a.out path/to/toml.hpp > single_include/toml.hpp"
              << std::endl;
    return 1;
  }
  ...
  for (const auto &entities :
       std::filesystem::directory_iterator(include_path / "toml11" / "fwd")) {
    if (!entities.is_regular_file()) {
      continue;
    }
    std::string fname = entities.path().filename().string();
    if (fname.ends_with("_fwd.hpp")) {
      for (const auto c : "_fwd.hpp"s) {
        fname.pop_back();
        (void)c;
      }
  ...
  for (const auto &entities :
       std::filesystem::directory_iterator(include_path / "toml11" / "impl")) {
    if (!entities.is_regular_file()) {
      continue;
    }
    std::string fname = entities.path().filename().string();
    if (fname.ends_with("_impl.hpp")) {
      for (const auto c : "_impl.hpp"s) {
        fname.pop_back();
        (void)c;
      }
  ...
  for (const auto &fname : input.includes) {
    ...
    std::cerr << "file " << fname << " has " << files.at(fname).content.size()
              << " lines." << std::endl;
  }
  ...
  for (const auto &[fname, file] : files) {
    for (auto incl : file.includes) {
      ...
      if (incl_stem.ends_with("_fwd")) {
        for (const char c : "_fwd"s) {
          incl_stem.pop_back();
          (void)c;
        }

        // include original file instaed
        incl = incl.parent_path() / ".." /
               std::filesystem::path(incl_stem + ".hpp");
      } else if (incl_stem.ends_with("_impl")) {
        for (const char c : "_impl"s) {
          incl_stem.pop_back();
          (void)c;
        }
