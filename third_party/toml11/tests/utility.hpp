...
template <typename... Ts> std::vector<std::string> comments(Ts... args) {
...
void toml11_test_parse_success(std::string in, const T &out,
  ...
  const std::vector<std::string> com_actual(val.comments().begin(),
                                            val.comments().end());
...
void toml11_test_parse_failure(F fn, std::string in,
                               toml::detail::context<TC> ctx) {
