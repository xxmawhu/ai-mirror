...
struct corge {
  ...
  template <typename TC> toml::basic_value<TC> into_toml() const {
...
namespace toml {
template <> struct from<extlib::foo> {
...
template <> struct into<extlib::foo> {
  template <typename TC>
  static toml::basic_value<TC> into_toml(const extlib::foo &f) {
    return toml::basic_value<TC>(
        typename toml::basic_value<TC>::table_type{{"a", f.a}, {"b", f.b}});
  }
};

template <> struct from<extlib::baz> {
...
template <> struct into<extlib::qux> {
  template <typename TC>
...
struct bar {
  ...
  template <typename TC> void from_toml(const toml::basic_value<TC> &v) {
    this->a = toml::find<int>(v, "a");
    this->b = toml::find<std::string>(v, "b");
    return;
  }
...
namespace toml {
template <> struct from<extlib2::foo> {
  template <typename TC>
...
template <> struct into<extlib2::foo> {
  template <typename TC>
...
template <> struct from<extlib2::baz> {
  template <typename TC>
...
template <> struct into<extlib2::qux> {
  template <typename TC>
  static toml::basic_value<TC> into_toml(const extlib2::qux &f) {
    return toml::basic_value<TC>{{"a", f.a}, {"b", f.b}};
  }
};
} // namespace toml
...
TEST_CASE("test_conversion_via_constructor") {
  ...
  {
    const toml::ordered_value v(
        toml::ordered_table{{"a", 42}, {"b", "foobar"}});
...
TEST_CASE("test_recursive_conversion") {
  ...
  {
    const toml::ordered_value v(
        toml::ordered_array{toml::ordered_table{{"a", 42}, {"b", "baz"}},
  ...
  // via constructor
  {
    const toml::value v(toml::array{toml::table{{"a", 42}, {"b", "baz"}},
                                    toml::table{{"a", 43}, {"b", "qux"}},
                                    toml::table{{"a", 44}, {"b", "quux"}},
                                    toml::table{{"a", 45}, {"b", "foobar"}}});
  ...
  {
    const toml::value v(
        toml::table{{"0", toml::table{{"a", 42}, {"b", "baz"}}},
...
TEST_CASE("test_optional_conversion_via_macro") {
  ...
  {
    const toml::ordered_value v(toml::ordered_table{{"b", "baz"}});
  ...
  {
    const toml::value v(
        toml::table{{"b", "bar.b"}, {"f", toml::table{{"a", 42}}}});
  ...
  {
    const toml::ordered_value v(toml::ordered_table{
        {"a", 42}, {"f", toml::ordered_table{{"b", "foo.b"}}}});
