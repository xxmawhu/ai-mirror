...
TEST_CASE("testing basic string") {
  ...
  toml11_test_parse_success<toml::value_t::string>(
      ...
      string_fmt(), ctx);
  toml11_test_parse_success<toml::value_t::string>(
      "\"192.168.1.1\"", "192.168.1.1", comments(), string_fmt(), ctx);

  toml11_test_parse_success<toml::value_t::string>(
      "\"\xE4\xB8\xAD\xE5\x9B\xBD\"", "\xE4\xB8\xAD\xE5\x9B\xBD", comments(),
      string_fmt(), ctx);

  toml11_test_parse_success<toml::value_t::string>(
      "\"You'll hate me after this - #\"", "You'll hate me after this - #",
      comments(), string_fmt(), ctx);
  toml11_test_parse_success<toml::value_t::string>(
...
TEST_CASE("testing multiline basic string") {
  ...
  toml11_test_parse_success<toml::value_t::string>(
      ...
      "Here are three quotation marks: \"\"\".", comments(), string_fmt(false),
      ctx);
...
TEST_CASE("testing literal_string") {
  ...
  toml11_test_parse_success<toml::value_t::string>(
      ...
      comments(), string_fmt(), ctx);
  toml11_test_parse_success<toml::value_t::string>(
      "'<\\i\\c*\\s*>'", "<\\i\\c*\\s*>", comments(), string_fmt(), ctx);
...
TEST_CASE("testing ml_literal_string") {
  ...
  toml11_test_parse_success<toml::value_t::string>(
      ...
      "The first newline is\ntrimmed in raw strings.\n   All other "
      "whitespace\n   is preserved.\n",
      comments(), string_fmt(true), ctx);

  toml11_test_parse_success<toml::value_t::string>(
      "''''That's still pointless', she said.'''",
      "'That's still pointless', she said.", comments(), string_fmt(false),
      ctx);
