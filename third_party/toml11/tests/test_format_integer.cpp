...
TEST_CASE("testing decimal") {
  ...
  CHECK_EQ("123_456_789",
           toml::format(toml::value(123456789, decimal_fmt(9, 3))));
}
...
TEST_CASE("testing hex") {
  ...
  CHECK_EQ("0xdeadbeef",
           toml::format(toml::value(0xDEADBEEF, hex_fmt(false, 8, 0))));
  CHECK_EQ("0xdead_beef",
           toml::format(toml::value(0xDEADBEEF, hex_fmt(false, 8, 4))));
  ...
  CHECK_EQ("0xDEADBEEF",
           toml::format(toml::value(0xDEADBEEF, hex_fmt(true, 8, 0))));
  CHECK_EQ("0xDEAD_BEEF",
           toml::format(toml::value(0xDEADBEEF, hex_fmt(true, 8, 4))));
...
TEST_CASE("testing oct") {
  ...
  CHECK_EQ("0o644",
           toml::format(toml::value(64 * 6 + 8 * 4 + 4, oct_fmt(3, 0))));
  CHECK_EQ("0o7_7_7",
           toml::format(toml::value(64 * 7 + 8 * 7 + 7, oct_fmt(3, 1))));
  CHECK_EQ("0o000644",
           toml::format(toml::value(64 * 6 + 8 * 4 + 4, oct_fmt(6, 0))));
}
...
TEST_CASE("testing decimal with suffix") {
  ...
  CHECK_EQ("1234_kcal",
           toml::format(toml::value(1234, decimal_fmt(0, 0, "kcal")), sp));
  CHECK_EQ("1_2_3_4_μm",
           toml::format(toml::value(1234, decimal_fmt(4, 1, "μm")), sp));
}
