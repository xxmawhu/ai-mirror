...
static const std::set<std::string> VALID_MONTHS = {"Jan", "Feb", "Mar", "Apr",
                                                   "May", "Jun", "Jul", "Aug",
                                                   "Sep", "Oct", "Nov", "Dec"};

static std::vector<std::string> read_last_n_lines(const std::string &path,
                                                  int n) {
  ...
  while (running_.load()) {
    ...
    while (std::getline(log_file, line)) {
      ...
      for (const auto &keyword :
           {"Accepted", "Failed", "session opened", "session closed"}) {
        ...
        if (pos != std::string::npos) {
          ...
          if (user_pos != std::string::npos) {
            ...
            if (!utils::validate_username(user)) {
              utils::get_logger()->warn("AuthMonitor: skipping event with "
                                        "invalid username '{}' in auth log",
                                        user);
              break;
            }
            event.username = user;
    ...
    if (lines_read == 0 && !log_file.eof() && log_file.fail()) {
      ...
      if (consecutive_errors >= max_consecutive_errors) {
        utils::get_logger()->error(
            "AuthMonitor: {} consecutive read errors, stopping",
...
void AuthMonitor::stop() { running_.store(false); }
...
std::vector<AuthEvent> AuthMonitor::get_recent_events(int count) {
  ...
  for (const auto &line : lines) {
    ...
    if (!event.action.empty()) {
      ...
      if (static_cast<int>(events.size()) >= count)
        break;
    }
  }
