...
# Note: executable scripts (scripts/*.sh, install.sh) use 700 (owner execute only)
...
# Check shell scripts in src/ only (must be 600, not executable)
# scripts/ and install.sh are executable hooks, use 700 (owner only)
while IFS= read -r -d '' file; do
	perms=$(stat -c '%a' "$file")
	if [[ "$perms" != "600" ]]; then
		echo "ERROR: $file has permissions $perms (expected 600)"
	...
	-- 'src/*.sh' 2>/dev/null | while IFS= read -r -d '' f; do
	[[ -f "$f" ]] && printf '%s\0' "$f"
done)
