...
int main() {
  ...
  // using member functions
  {
    ...
    assert(root.at("contributors").at(0).as_string() ==
           "Foo Bar <foo@example.com>");
    assert(root.at("contributors").at(1).at("name").as_string() == "Baz Qux");
    assert(root.at("contributors").at(1).at("email").as_string() ==
           "bazqux@example.com");
    assert(root.at("contributors").at(1).at("url").as_string() ==
  ...
  // using toml::find
  {
    ...
    const auto nested_arrays_of_ints =
        toml::find<std::vector<std::vector<int>>>(root,
    ...
    const auto numbers =
        toml::find<std::tuple<double, double, double, int, int, int>>(
            root, "numbers");
